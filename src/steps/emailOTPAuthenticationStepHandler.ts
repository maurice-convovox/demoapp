...
export class EmailOTPAuthenticationStepHandler implements StepHandler {
  ...
  protected setOtpCodeLength(otpCodeLength?: number) {
    if (otpCodeLength) {
      ...
      getElement<HTMLDivElement>(
        `#step-warning`,
      )!.innerHTML = `The OTP code length was not provided. It has been set to the default value of ${this.otpCodeLength}.`;
    }
  }
  ...
  protected generateHtml() {
    ...
    return sanitizeHtml(`
      <div class="alert info">Email OTP</div>
      <form class="column">
        ${otpComponent}
  ...
  private setEventListeners(submitStep: StepResolver) {
    ...
    otp.inputs.forEach((input: Element) =>
      input.addEventListener('input', () => {
        // Get the otp value and remove the empty chars
